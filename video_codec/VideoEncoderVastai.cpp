...
bool VideoEncoderVastai:: InitCtxParams()
{
    ...
    int ret = avOptSet(m_encoderCtx->priv_data, "vast-params", "preset=bronze_quality:miniGopSize=1:lookaheadLength=0:llRc=5", 0);
    if (ret != 0) { 
...
bool VideoEncoderVastai::InitFrameData(const uint8_t *src)
{
    ...
    VastFrameInfo vastFrame;
    vastFrame.width = m_encoderCtx->width;
    vastFrame.height = m_encoderCtx->height;
    vastFrame.format = AV_PIX_FMT_YUV420P;
    auto avFrameAllocVastai = reinterpret_cast<AvFrameAllocVastaiFunc>(g_funcMap_avutil[AV_FRAME_ALLOC_VASTAI]);
    m_swFrame = avFrameAllocVastai(m_encoderCtx->hw_frames_ctx, &vastFrame);
    ...
    for (int i = 0; i < NUM_OF_PLANES; i++) {
        if (i > 0) {
            tmpHeight = m_encoderCtx->height / COMPRESS_RATIO;
            tmpWidth = m_encoderCtx->width / COMPRESS_RATIO;
        } else {
            tmpHeight = m_encoderCtx->height;
            tmpWidth = m_encoderCtx->width;
        }
        for (int j = 0; j < tmpHeight; j++) {
            memcpy(m_swFrame->data[i], src, tmpWidth);
            m_swFrame->data[i] = m_swFrame->data[i] + m_swFrame->linesize[i];
            src = src + tmpWidth;
        }
    }
    ...
    auto avHwframeTransferData = reinterpret_cast<AvHwframeTransferDataFunc>(g_funcMap_avutil[AV_HWFRAME_TRANSFER_DATA]);
    if (avHwframeTransferData(m_hwFrame, m_swFrame, 0) != VIDEO_ENCODER_SUCCESS) {
        ERR("avHwframeTransferData failed.Error code: %d", ret);
        return false;
    }
