...
namespace {
    ...
    const std::string ENCODER_TYPE_VASTAI_H264 = "h264_ni_quadra_enc";
    const std::string ENCODER_TYPE_VASTAI_H265 = "h265_ni_quadra_enc";
...
VideoEncoderQuadra::VideoEncoderQuadra(QuaCodecType codecType)
{
    ...
    INFO("VideoEncoderQuadra constructed %s", (m_codec == ENCODER_TYPE_VASTAI_H264) ? "h.264" : "h.265");
}
...
bool VideoEncoderQuadra::InitCtxParams()
{
    ...
    if (m_codec == ENCODER_TYPE_VASTAI_H264) {
        profileValue = profileOpt + g_transProfile[m_encParams.profile];
    } else if (m_codec == ENCODER_TYPE_VASTAI_H265) {
    ...
    int ret = avOptSet(m_encoderCtx->priv_data, xcoderParams.c_str(), "gopPresetIdx=9:RcEnable=1:lowDelay=1", 0);
    if (ret != 0) {
